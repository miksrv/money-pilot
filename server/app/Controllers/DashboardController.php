...
class DashboardController extends ApplicationBaseController
{
    ...
    public function monthlySpending(): ResponseInterface
    {
        ...
        $currentMonthData  = $this->buildCumulativeSeries($userId, $currentMonthStr, $daysInCurrentMonth, $currentDay);
        $previousMonthData = $this->buildCumulativeSeries($userId, $previousMonthStr, $daysInPreviousMonth, $daysInPreviousMonth);
     ...
     */
    private function buildCumulativeSeries(string $userId, string $month, int $daysInMonth, int $fillToDay): array
    {
        $startDate = $month . '-01';
